		...
		while True:
			try:
				...
				for wallet in wallets:
					if wallet["balance"] > 0:
						print(
							f"{bcolors.GREEN}[+{wallet.get('provider','?')}] {wallet['address']} : {float(wallet['balance'])/1e8} BTC : {wallet['private']}{bcolors.RESET}",
							flush=True,
						)
						# save wallet to file
						...
						if args.verbose:
							print(
								f"{bcolors.RED}[-{wallet.get('provider','?')}] {wallet['address']} : {float(wallet['balance'])/1e8} BTC : {wallet['private']}{bcolors.RESET}",
								flush=True,
							)
						else:
							print(
